# Series 2.0 - AI-Powered Social Networking Platform
...
Series 2.0 is a social networking platform that:
...
└────────────────────┬────────────────────────────────────────┘
      ...
      ▼             ▼           ▼            ▼
┌──────────┐  ┌──────────┐  ┌──────────┐  ┌──────────┐
│ Chatbot  │  │ Firebase │  │ Matching │  │ Series   │
...
- **Series iMessage API** credentials
...
└── Documentation/
    ...
    ├── API_DOCUMENTATION.md     # Series API docs
...
Generate response with Claude AI
  ...
  - Personalized tone
  - Short (1-2 sentences)
  - Suggests Series.so when relevant
    ↓
Send reply via Series API
    ↓
...
- **Helpful**: Suggests https://tv.series.so/ when users mention boredom
...
User: "I'm bored"
Bot: "You should check out https://tv.series.so/ - they have some great new series to watch!"
...
# Run API examples
npm run api-examples   # Test Series API
```
...
# Run container
docker run -p 8000:8000 --env-file .env series
```
...
```
https://series-xxxxx-uc.a.run.app
```
...
- **[API_DOCUMENTATION.md](API_DOCUMENTATION.md)** - Series iMessage API reference
- **[API_ENDPOINTS.md](API_ENDPOINTS.md)** - All server endpoints
...
3. Ensure Series API credentials are correct
...
- **Series.so** for iMessage API integration
